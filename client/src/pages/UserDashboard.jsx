...
const AppointmentBooking = ({ predictionResult, onBook }) => {
  ...
  const searchDoctors = async () => {
    ...
    const mapsEmbedUrl = `https://www.google.com/maps/embed/v1/search?key=AIzaSyD-9tSrke72NouTzTEsYqUSS_M5MwUPDQg&q=${query}`;
    setMapUrl(mapsEmbedUrl);
    await new Promise(r => setTimeout(r, 1200));
    const mockDoctors = [
      { id: 1, name: `Dr. Sarah Johnson`, specialty: searchQuery, rating: 4.8, reviews: 127, distance: '0.8 mi', address: `123 Medical Center Dr, ${location}`, phone: '(555) 101-2020', available: true, nextSlot: 'Tomorrow', insurance: ['BlueCross', 'Aetna', 'UnitedHealth'] },
      { id: 2, name: `Dr. Michael Chen`, specialty: searchQuery, rating: 4.6, reviews: 89, distance: '1.2 mi', address: `456 Health Plaza, ${location}`, phone: '(555) 202-3030', available: true, nextSlot: 'Today', insurance: ['Cigna', 'Humana', 'Medicare'] },
      { id: 3, name: `Dr. Emily Rodriguez`, specialty: searchQuery, rating: 4.9, reviews: 203, distance: '2.1 mi', address: `789 Wellness Ave, ${location}`, phone: '(555) 303-4040', available: true, nextSlot: 'In 2 days', insurance: ['BlueCross', 'Cigna', 'Aetna'] },
    ];
  ...
  const confirmBooking = () => {
    ...
    const appt = {
      id: `apt_${Date.now()}`, doctor: selectedDoctor.name, specialty: selectedDoctor.specialty,
...
const HealthRecordsTab = ({ userData, onVitalsUpdate, onRecordAdd, onDiseaseAnalyticsUpdate }) => {
  ...
  const applyAiAnalysis = () => {
    ...
    const { vitals, medications, diagnosis, predictedConditions, severity, specialistRecommended } = aiAnalysis;
...
const Dashboard = () => {
  ...
  // NEW: Handle disease analytics from AI prescription analysis
  ...
  const handleRecordAdd = (record) => { setHealthRecords(prev => [record, ...prev]); };
  const handleAppointmentBook = (appt) => {
  ...
  return (
    <div className="min-h-screen text-slate-800" style={{ background: '#f0f4f8', fontFamily: "'DM Sans', system-ui, sans-serif" }}>
      ...
      <div className="flex min-h-screen">
        ...
        <aside className={`${sidebarOpen ? 'w-64' : 'w-[72px]'} flex-shrink-0 transition-all duration-300 flex flex-col`}
          ...
          <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
            ...
            <div className="pt-3 border-t" style={{ borderColor: 'rgba(255,255,255,0.06)' }}>
              <button onClick={() => setShowBot(true)} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-2xl transition-all pulse-glow"
                style={{ background: 'linear-gradient(135deg, rgba(124,58,237,0.25), rgba(59,130,246,0.2))', border: '1px solid rgba(124,58,237,0.4)', color: '#a78bfa' }}>
                <span className="text-base flex-shrink-0">🤖</span>
                {sidebarOpen && <div><div className="font-bold text-xs">Disease Predictor Bot</div><div className="text-purple-400 text-[10px]">ML Model</div></div>}
              </button>
            </div>
          </nav>
        ...
        <main className="flex-1 flex flex-col overflow-hidden">
          <header className="h-16 flex items-center justify-between px-8 flex-shrink-0" style={{ background: 'white', borderBottom: '1px solid #e2e8f0', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
            ...
            <div className="flex items-center gap-3">
              <button onClick={() => setShowBot(true)} className="btn-primary flex items-center gap-2 px-4 py-2 text-sm">
                🤖 <span>Disease Predictor Bot</span>
              </button>
              ...
              <div className="space-y-6 max-w-7xl mx-auto">
                <div className="rounded-2xl p-6 relative overflow-hidden" style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e3a5f 60%, #0f4c81 100%)' }}>
                  <div className="relative flex items-center justify-between">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button onClick={() => setShowBot(true)} className="flex items-center gap-2 px-5 py-2.5 rounded-xl text-white font-bold text-sm" style={{ background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)' }}>
                        🤖 Disease Predictor Bot
                      </button>
                    </div>
                  </div>
                </div>
                ...
                <div className="card p-6">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                    {[
                      { icon: '🤖', label: 'Disease Predictor Bot', desc: 'ML-powered analysis', action: () => setShowBot(true), primary: true },
                  ...
                  <div className="card p-6">
                    ...
                    {recentPredictions.length === 0 && (
                      <div className="text-center py-8 text-slate-400">
                        ...
                        <button onClick={() => setShowBot(true)} className="btn-primary px-4 py-2 text-xs mt-3">Try MediBot</button>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {activeTab === 'symptoms' && (
              <div className="max-w-5xl mx-auto space-y-6">
                <div className="rounded-2xl p-5 flex items-center justify-between" style={{ background: 'linear-gradient(135deg, #4c1d95, #1e40af)', border: '1px solid rgba(139,92,246,0.3)' }}>
                  ...
                  <button onClick={() => setShowBot(true)} className="px-5 py-2.5 rounded-xl font-bold text-sm" style={{ background: 'rgba(255,255,255,0.2)', color: 'white', border: '1px solid rgba(255,255,255,0.3)' }}>Open MediBot</button>
                </div>
                <div className="grid lg:grid-cols-2 gap-6">
                  <div className="card p-6">
                  ...
                  <div className="card p-6">
                    ...
                    {predictionResult ? (
                      ...
                      <div className="flex flex-col items-center justify-center h-64 text-center">
                        ...
                        <button onClick={() => setShowBot(true)} className="btn-primary px-5 py-2.5 text-sm">🤖 Try MediBot Instead</button>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {showBot && <DiseasePredictorBot onClose={() => setShowBot(false)} />}
    </div>
  );
};
