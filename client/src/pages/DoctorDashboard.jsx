...
export default function DoctorDashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        const sqlData = await doctorApi.fetchDoctorDashboardData(user.email);
        if (sqlData) {
          existing.appointments = sqlData.appointments || [];
          existing.patients = sqlData.patients || [];
          existing.prescriptions = sqlData.prescriptions || [];
          existing.tasks = sqlData.tasks || [];
          existing.invoices = sqlData.invoices || [];
          existing.reminders = sqlData.reminders || [];
          existing.diagnoses = sqlData.diagnoses || [];
          existing.vitals = sqlData.vitals || [];
          existing.notes = sqlData.notes || [];
          
          const monthly = Array(12).fill(null).map(() => ({ completed: 0, missed: 0, cancelled: 0, scheduled: 0 }));
          existing.appointments.forEach(a => {
      ...
      } catch (err) { console.error("SQL Fetch failed:", err); }
  ...
  const addPatient = async (e) => {
    e.preventDefault();
    const pat = { ...patForm, doctorEmail: authUser.email, lastVisit: today };
    try {
      const saved = await doctorApi.createSqlPatient(pat);
      if (saved) {
        updateDB(d => ({ ...d, patients: [saved, ...d.patients], notifications: [addNotif('patient', 'Patient Registered', saved.name + ' added to your roster'), ...d.notifications] }));
        showToast('Patient added to SQL database!');
  ...
  const addAppointment = async (e) => {
    ...
    if (!pat) return;
    const apt = { ...aptForm, patientName: pat.name, doctorEmail: authUser.email };
    try {
      const saved = await doctorApi.createSqlAppointment(apt);
      if (saved) {
        updateDB(d => ({ ...d, appointments: [saved, ...d.appointments], notifications: [addNotif('appointment', 'Appointment Scheduled', `${pat.name} on ${fmt(aptForm.date)}`), ...d.notifications] }));
        showToast('Appointment saved to SQL!');
        setAptForm(blankApt); setModal(null);
      }
    } catch (err) { showToast('Failed to save appointment', 'error'); }
  };

  const updateAptStatus = async (aptId, status) => {
    try {
      const saved = await doctorApi.updateSqlAppointmentStatus(aptId, status);
      if (saved) {
        updateDB(d => {
        ...
        showToast(`Appointment status updated in SQL: ${status}`);
      }
  ...
  const addPrescription = async (e) => {
    ...
    if (!pat) return;
    const rx = { ...rxForm, patientName: pat.name, doctorEmail: authUser.email, date: today, status: 'Active' };
    try {
      const saved = await doctorApi.createSqlPrescription(rx);
      if (saved) {
        updateDB(d => ({ ...d, prescriptions: [saved, ...d.prescriptions], totalPrescriptions: (d.totalPrescriptions || 0) + 1, notifications: [addNotif('prescription', 'Prescription Issued', `${rxForm.medication} for ${pat.name}`), ...d.notifications] }));
        showToast('Prescription saved to SQL!');
        setRxForm(blankRx); setModal(null);
      }
    } catch (err) { showToast('Failed to save prescription', 'error'); }
  };
  ...
  const addDiagnosis = async (e) => {
    ...
    if (!pat) return;
    const diag = { ...diagForm, patientName: pat.name, doctorEmail: authUser.email, date: today };
    try {
      const saved = await doctorApi.createSqlDiagnosis(diag);
      if (saved) {
        updateDB(d => ({ ...d, diagnoses: [saved, ...d.diagnoses], patients: d.patients.map(p => p.id === diagForm.patientId ? { ...p, condition: diagForm.diagnosis, lastVisit: today } : p), notifications: [addNotif('lab', 'Diagnosis Recorded', `${diagForm.diagnosis} for ${pat.name}`), ...d.notifications] }));
        showToast('Diagnosis saved to SQL!');
        setDiagForm(blankDiag); setModal(null);
      }
    } catch (err) { showToast('Failed to save diagnosis', 'error'); }
  };
  ...
  const addVitals = async (e) => {
    ...
    if (!pat) return;
    const vit = { ...vitForm, patientName: pat.name, doctorEmail: authUser.email, date: today };
    try {
      const saved = await doctorApi.createSqlDoctorVital(vit);
      if (saved) {
        updateDB(d => ({ ...d, vitals: [saved, ...d.vitals] }));
        showToast('Vitals saved to SQL!');
        setVitForm(blankVit); setModal(null);
      }
    } catch (err) { showToast('Failed to save vitals', 'error'); }
  };
  ...
  const addNote = async (e) => {
    ...
    if (!pat) return;
    const note = { ...noteForm, patientName: pat.name, doctorEmail: authUser.email, date: today };
    try {
      const saved = await doctorApi.createSqlDoctorNote(note);
      if (saved) {
        updateDB(d => ({ ...d, notes: [saved, ...d.notes] }));
        showToast('Note saved to SQL!');
  ...
  const addTask = async () => {
    ...
    const task = { title: newTaskTitle.trim(), priority: newTaskPri, doctorEmail: authUser.email, done: false, due: '' };
    try {
      const saved = await doctorApi.createSqlTask(task);
      if (saved) {
        updateDB(d => ({ ...d, tasks: [saved, ...d.tasks] }));
        setNewTaskTitle(''); showToast('Task saved to SQL!');
      }
  ...
  const addInvoice = async (e) => {
    ...
    try {
      const saved = await doctorApi.createSqlInvoice(inv);
      if (saved) {
        updateDB(d => ({
          ...d,
          invoices: [saved, ...(d.invoices || [])],
          notifications: [addNotif('billing', 'Invoice Created', `${saved.invoiceNo} · $${saved.total} for ${pat.name}`), ...d.notifications],
        }));
        showToast(`Invoice ${saved.invoiceNo} saved to SQL!`);
        setInvForm({ ...blankInvoice, services: [{ desc: '', qty: 1, rate: '' }] });
        setModal(null);
      }
    } catch (err) { showToast('Failed to save invoice', 'error'); }
  };
  ...
  const addReminder = async (e) => {
    ...
    try {
      const saved = await doctorApi.createSqlReminder(rem);
      if (saved) {
        updateDB(d => ({
          ...d,
          reminders: [saved, ...(d.reminders || [])],
          notifications: [addNotif('reminder', 'Reminder Set', `${saved.title} for ${pat.name} on ${fmt(saved.dueDate)}`), ...d.notifications],
        }));
        showToast('Reminder saved to SQL!');
